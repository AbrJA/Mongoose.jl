...
function (mw::Cors)(request::Request, next::Function)
    ...
    if request.method === :options
        ...
        headers = Pair{String,String}[
            ...
            "Access-Control-Max-Age"       => string(mw.max_age_seconds),
            "Vary"                         => "Origin",
        ]
...
"""
    cors(; origins, allow_methods, allow_headers, max_age_seconds, allow_credentials)
...
- `allow_methods::String`: Allowed HTTP methods (default `"GET, POST, PUT, PATCH, DELETE, OPTIONS"`).
- `allow_headers::String`: Allowed request headers (default `"Content-Type, Authorization"`).
- `max_age_seconds::Int`: Preflight cache duration in seconds (default `86400`).
...
function cors(;
    origins="*",
    allow_methods::String="GET, POST, PUT, PATCH, DELETE, OPTIONS",
