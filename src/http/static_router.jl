...
"""
    StaticHttpRouter — Base type for static HTTP app types generated by @router.
"""
abstract type StaticHttpRouter <: AbstractRouter end
...
macro router(app_type::Symbol, block)
    ...
    handler_definitions = esc(quote
        function $async_handler_sym(conn::Ptr{Cvoid}, ev::Cint, ev_data::Ptr{Cvoid})::Cvoid
            ...
            server = Base.unsafe_pointer_to_objref(fn_data)::Mongoose.AsyncServer{Mongoose.HttpRouter, $app_type, <:Mongoose.AbstractWsRouter}
        ...
        function $sync_handler_sym(conn::Ptr{Cvoid}, ev::Cint, ev_data::Ptr{Cvoid})::Cvoid
            ...
            server = Base.unsafe_pointer_to_objref(fn_data)::Mongoose.SyncServer{Mongoose.HttpRouter, $app_type, <:Mongoose.AbstractWsRouter}
            Mongoose._invoke_dispatch(server, ev, conn, ev_data)
            return nothing
        end
    ...
    quote
        struct $(esc(app_type)) <: Mongoose.StaticHttpRouter end

        $handler_definitions

        # --- Static Router Dispatch ---
        function Mongoose.static_dispatch(::$(esc(app_type)), $(req_sym)::Mongoose.Request)::Mongoose.Response
            ...
            return Mongoose.Response(404, "Content-Type: text/plain\r\n", "404 Not Found")
        end
