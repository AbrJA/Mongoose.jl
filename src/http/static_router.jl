...
"""
    NoStaticRouter — Sentinel type indicating no static app is configured.
"""
struct NoStaticRouter <: StaticRouter end
...
macro router(app_type::Symbol, block)
    ...
    handler_definitions = esc(quote
        function $async_handler_sym(conn::Ptr{Cvoid}, ev::Cint, ev_data::Ptr{Cvoid})::Cvoid
            ...
            server = Base.unsafe_pointer_to_objref(fn_data)::Mongoose.AsyncServer{Mongoose.Router, $app_type, <:Mongoose.AbstractWsRouter}
        ...
        function $sync_handler_sym(conn::Ptr{Cvoid}, ev::Cint, ev_data::Ptr{Cvoid})::Cvoid
            ...
            server = Base.unsafe_pointer_to_objref(fn_data)::Mongoose.SyncServer{Mongoose.Router, $app_type, <:Mongoose.AbstractWsRouter}
            Mongoose._invoke_dispatch(server, ev, conn, ev_data)
            return nothing
        end
